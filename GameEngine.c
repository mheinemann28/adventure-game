...
void list()
{
	...
	printf("\t\thelp: actions\n\n");
	printf("\t\tinventory: items, bag, objects\n\n");
	printf("\t\thit: smack, fight, attack, charge, punch, kick, hurt, swing, give, use, unlock, shine, throw\n\n");
	printf("\t\topen:\n\n");
	printf("\t\tmove: push\n\n");
	printf("\t\tdrop: leave, place, put\n\n");
}
...
void moveFeature(struct parsed_command pc, struct Room * room) {
	...
	for (j = 0; j < room->numExits; j++) {
		for (i = 0; i < MAX_FEATURES; i++) {
			if (strcmp(room->feature[i].name, pc.noun1) == 0) {
				if (strcmp(room->blockedBy[j], room->feature[i].name) == 0) {
				...
				else if (i == MAX_FEATURES - 1 && strcmp(room->feature[i - 1].enemy, "Yes") == 0) {
		...
		else if (strcmp(pc.noun1, room->feature[i].name) == 0)
		{
			...
			for (n = 0; n < 9; n++) {
				if (strcmp(objArray[n].usedFor, room->feature[i].name) == 0) {
					...
					if (invArray.invCount == 0 && strlen(pc.noun2) > 0) {
